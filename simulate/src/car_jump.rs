use common::{kinematics::kinematic, math::quadratic, physics::car_roof_axis, rl};
...
pub fn jump_duration(rot: &UnitQuaternion<f32>, target_dist: f32) -> Option<f32> {
    ...
    if let Some([_m, p]) = quadratic(a.dot(&axis), (2.0 * v_0).dot(&axis), -2.0 * target_dist) {
        if p < rl::CAR_JUMP_FORCE_TIME {
            return Some(p);
        }
    }
    ...
    let (d_0, v_0) = kinematic(Point3::origin(), v_0, a, rl::CAR_JUMP_FORCE_TIME);
    let a = Vector3::z() * rl::GRAVITY;
    if let Some([_m, p]) = quadratic(
        a.dot(&axis),
        (2.0 * v_0).dot(&axis),
        (2.0 * d_0 - Point3::origin()).dot(&axis) - 2.0 * target_dist,
    ) {
        return Some(p);
    }
...
mod tests {
    ...
    fn test_jump_duration() {
        ...
        assert_eq!(jump_duration(&flat, 100.0).unwrap(), 0.058722112);
        assert_eq!(jump_duration(&flat, 200.0).unwrap(), 0.38079262);
        assert_eq!(jump_duration(&flat, 220.0).unwrap(), 0.50065047);
