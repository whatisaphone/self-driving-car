...
use routing::{
    ...
    segments::{NullSegment, SimpleArc, Straight},
};
...
impl RoutePlannerCloneBox for TurnTowards {
...
impl RoutePlanner for TurnTowards {
    fn plan(&self, start: &CarState, _scenario: &Scenario) -> Result<RouteStep, RoutePlanError> {
        ...
        let turn_rot = start_forward_axis.rotation_to(self.target_loc - start_loc);
        ...
        let [tangent1, tangent2] =
            match circle_point_tangents(turn_center, turn_radius, self.target_loc) {
                Some(x) => x,
                None => {
        ...
        let t1_dot = t1_vel.dot(&(self.target_loc - tangent1));
        let t2_dot = t2_vel.dot(&(self.target_loc - tangent2));
